...
from datagen_v2 import DataGenerator3D, MultiModalDataGenerator, NiftiBraTSDataGenerator, T2WeightedDICOMDataGenerator, sanity_check_3d_generator
import pydicom
...
def train_3d_unet(config, data_gen_train=None, data_gen_valid=None):
    ...
    print(f"Use T2-weighted DICOM data: {config.get('use_t2weight', False)}")

    # Use provided data generators if available, otherwise create new ones
    if data_gen_train is not None and data_gen_valid is not None:
        print("Using provided data generators")
    elif config.get('use_t2weight', False):
        # Use T2-weighted DICOM data generator
        # Create a temporary generator to get the split
        temp_gen = T2WeightedDICOMDataGenerator(
        ...
        # Training generator
        data_gen_train = T2WeightedDICOMDataGenerator(
            data_path=config['data_path'],
            batch_size=config['batch_size'],
            input_dims=config['input_shape'],
            output_dims=config['output_shape'],
        ...
        data_gen_valid = T2WeightedDICOMDataGenerator(
            data_path=config['data_path'],
            batch_size=config['batch_size'],
            input_dims=config['input_shape'],
            output_dims=config['output_shape'],
    ...
    callbacks = [
        ...
        tf.keras.callbacks.ModelCheckpoint(
            'best_weights_t2weight.weights.h5',
            monitor="val_loss",
            verbose=1,
            save_best_only=True,
    ...
    pd.DataFrame(history.history).to_csv('model_history_t2weight.csv')
    print("Training completed. History saved to model_history_t2weight.csv")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Train 3D Multi-modal Fat Suppression v2 - T2-weighted DICOM Version')
    parser.add_argument('--data_path', type=str, default=r'C:\Users\rszub\Documents\Brain Dataset\Brain T1&T2 Weighted Dataset\Neurohacking_data-0.0\BRAINIX\DICOM',
                       help='Path to T2-weighted DICOM dataset directory')
    parser.add_argument('--valid_path', type=str, default=r'C:\Users\rszub\Documents\Brain Dataset\Brain T1&T2 Weighted Dataset\Neurohacking_data-0.0\BRAINIX\DICOM',
                       help='Path to T2-weighted DICOM validation data')
    ...
    parser.add_argument('--use_t2weight', action='store_true',
                       help='Use T2-weighted DICOM dataset format (T2 -> FLAIR T2-weighted)')
    ...
    config = {
        ...
        'use_t2weight': args.use_t2weight
    }

    # Create data generator for training
    if config.get('use_t2weight', False):
        print("Initializing T2-weighted DICOM data generator...")
        data_gen_train = T2WeightedDICOMDataGenerator(
            data_path=config['data_path'],
            batch_size=config['batch_size'],
            input_dims=config['input_shape'],
            output_dims=config['output_shape'],
            validation_split=0.0  # We'll handle validation manually
        )

        # Check if generator has any data
        print(f"Training generator length: {len(data_gen_train)}")
        if len(data_gen_train) == 0:
            print("ERROR: Training generator has length 0!")
            print("This means no valid T2/FLAIR pairs were found.")
            print("Please check your dataset structure and folder names.")
            return None

        # For single dataset, use same generator for validation
        data_gen_valid = data_gen_train

    # Run sanity check if requested
    if args.sanity_check:
        print("Running data generator sanity check...")
        sanity_check_3d_generator(data_gen_train, num_samples=2)
        return None  # Exit after sanity check

    # Rest of the training code...
    if not config.get('use_t2weight', False):
        # Handle other generator types if needed
        pass

    # Train the model
    if config.get('use_t2weight', False):
        history = train_3d_unet(config, data_gen_train, data_gen_valid)
    else:
